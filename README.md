...
- **Data Landing Zone:** ADLS Gen2 is partitioned into a storage container optimized for batch-driven file processing
...
- **Transformation & Lakehouse Modeling (Synapse Data Engineering):** Apache Spark Notebooks processing, cleansing, and graduating raw files to Delta Lake format (**`2-silver/` shortcut**), then further transforming and materializing curated Star-Schema fact and dimension tables into the **`3-gold/` shortcut**
- **Ad-Hoc SQL Analysis (SQL Analytics Endpoint):** An autogenerated, read-only T-SQL gateway on top of the Lakehouse Gold Delta tables, allowing Enterprise Data Analysts to execute direct, high-performance queries using standard client tools (SSMS / Azure Data Studio)
...
- **Predictive AI (Synapse Data Science):** An expansion to the data analytics capabilities of the platform, to train Machine Learning models utilizing curated historical Gold data features
